...
const ingresos = mockMovimientos
  .filter(
    (m) =>
      mockCategorias.find((c) => c.idCategoria === m.idCategoria)
        ?.idTipoMovimiento === 1,
  )
...
function PresupuestoBar({
  ...
  const color =
    porcentaje >= 90 ? "#f07060" : porcentaje >= 70 ? "#f0a060" : "#3ecf8e";
        ...
        >
          ...
          <div className="flex flex-col">
            {movimientosRecientes.map((m) => {
              const categoria = mockCategorias.find(
                (c) => c.idCategoria === m.idCategoria,
              );
              const metodo = mockMetodosPago.find(
                (mp) => mp.idMetodoPago === m.idMetodoPago,
              );
